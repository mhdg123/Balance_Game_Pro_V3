...
public class MemberDAO {
	...
	// 로그인 SQL
	private static final String LOGIN = "SELECT LOGIN_ID, ROLE, COIN, NICKNAME, COMMENT_STATUS FROM MEMBER WHERE LOGIN_ID = ? AND MEMBER_PASSWORD = ? ";
	...
	// 소셜 로그인
	private static final String SOCIAL_LOGIN = "SELECT LOGIN_ID, COIN, NICKNAME, COMMENT_STATUS FROM MEMBER WHERE LOGIN_ID=?";
...
class MemberRowMapperLogin implements RowMapper<MemberDTO> {
	...
	public MemberDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		member.setWriteStatus(rs.getString("COMMENT_STATUS"));
		return member;
	}
}
