...
public class PageInfoDAO {
	...
	private static final String SELECTALL_ADMIN_APPROVED_QUESTIONS = "SELECT Q.QUESTION_ID, Q.TITLE, Q.WRITER, Q.EXPLANATION, Q.QUESTION_DATE FROM QUESTION Q WHERE QUESTION_ACCESS = 'T' LIMIT ?, ?";

	// 문제목록 (관리자가 사용하는 승인하지 않은 전체 문제 조회)
	private static final String SELECTALL_ADMIN_UNAPPROVED_QUESTIONS = "SELECT Q.QUESTION_ID, Q.TITLE, Q.WRITER, Q.EXPLANATION, Q.QUESTION_DATE FROM QUESTION Q WHERE QUESTION_ACCESS = 'F' LIMIT ?, ?";
	...
	// 회원 랭킹 (결제금액으로 조회)
	private static final String SELECTALL_RANKING_ADMIN = "SELECT M.LOGIN_ID, M.NICKNAME,  P.PAYMENT_DATE, IFNULL(SUM(P.AMOUNT), 0) AS TOTAL, "
